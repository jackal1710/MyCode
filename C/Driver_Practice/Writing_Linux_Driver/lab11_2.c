...
{
	struct mydev_structure* dev_ptr= (struct mydev_structure*) data;
	pr_info("TIMER: process %d: %d\n", current->pid, (int) dev_ptr->val);
	kfree(dev_ptr);
}

static ssize_t mydev_write(struct file *filp, char const __user *buf, size_t count, loff_t *offp)
{
	struct mydev_structure* dev_ptr;

	if (count != sizeof(int))
		return -EFAULT;

	dev_ptr= (struct mydev_structure*) kmalloc(sizeof(struct mydev_structure), GFP_KERNEL);
	if (copy_from_user(&dev_ptr->val, buf, count))
		return -EFAULT;
	pr_info("PROCESS: process %d: %d\n", current->pid, (int) dev_ptr->val);
	
	//Create timer for each call
	init_timer(&dev_ptr->timer);
	dev_ptr->timer.function= mydev_timer_func;
	dev_ptr->timer.data= (unsigned long) dev_ptr;
	dev_ptr->timer.expires= jiffies + timeout*HZ;
	add_timer(&dev_ptr->timer);
