...
static int numr= 1, timeout1= 100, timeout2= 1000;
...
static unsigned long loading_time;
...
static void mydev_exit(void)
{
	...
	del_timer_sync(&timer1.timer);
	del_timer_sync(&timer2.timer);
...
static int __init mydev_init(void)
{
	...
	init_timer(&timer1.timer);
	init_timer(&timer2.timer);
	timer1.timeout= timeout1;
	timer2.timeout= timeout2;
	timer1.timer.function= mydev_timer_func;
	timer2.timer.function= mydev_timer_func;
	timer1.timer.data= (unsigned long) &timer1;
	timer2.timer.data= (unsigned long) &timer2;
	timer1.timer.expires= jiffies + timeout1;
	timer2.timer.expires= jiffies + timeout2;
	add_timer(&timer1.timer);
	add_timer(&timer2.timer);
	loading_time= jiffies;
	pr_info("Loading time: %lu\n", loading_time);
